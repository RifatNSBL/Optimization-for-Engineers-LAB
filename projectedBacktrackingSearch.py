...
def checkFunc(f, P, x: np.array, d: np.array, sigma, t):
    return f.objective(P.project(x + t * d)) <= f.objective(x) - (sigma / t) * np.linalg.norm(x - P.project(x - t * f.gradient(x)), 2) **2
...
def projectedBacktrackingSearch(f, P, x: np.array, d: np.array, sigma=1.0e-4, verbose=0):
    ...
    t = 1
    while not checkFunc(f, P, x, d, sigma, t):
        t /= 2
