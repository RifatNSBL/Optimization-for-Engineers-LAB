...
class leastSquaresObjective:
    ...
    def residual(self, p: np.array):
        ...
        for i in range(self.N):
            myResidual[i] = self.model.objective(self.xData[:, i].reshape((self.n, 1))) - self.fData[:, i]
        return myResidual
    ...
    def jacobian(self, p: np.array):
        ...
        self.model.setParameters(p)
        for i in range(self.N):
            myJacobian[i, :] = self.model.parameterGradient(self.xData[:, i].reshape(self.n, 1)).reshape(1, p.shape[0])
        return myJacobian