...
def derivativefreeBFGSDescent(f, x0: np.array, h, eps=1.0e-3, sigma=1.0e-4, verbose=0):
    ...
    while not satisfiesTermination:
        betak = np.minimum(1, 10 * h / np.linalg.norm(B @ grad_f_h, 2))
        d =  - betak * B @ grad_f_h
        t = 1
        linesearchCounter = 0
        ...
        if linesearchFail:
            ...
            dgk = SUC.SUCSimplexGradient(f, x + t * d, h) - SUC.SUCSimplexGradient(f, x, h)
            dx = t * d
            x = x + t * d
            grad_f_h = SUC.SUCSimplexGradient(f, x, h)
            r = dx - (B @ dgk)
            B += (r @ dx.T + dx @ r.T) / (dgk.T @ dx) - ((r.T @ dgk) * (dx @ dx.T) / (dgk.T @ dx) ** 2)
            loopCounter += 1
